// Here is the js for the query variables part
var variable = [];
	...
	$("#mainItem")
		...
		.click(function() {
			...
			$.each(variable, function(i, p) {
				...
				// 这里去掉了.val(p)的部分
				$(".conditional_statement_varaible_select").append(
			...
			clone
				.prepend(
					"<i class='material-icons checkedboxicon'>check_box</i><a name='selecteditem' value='x'></a>"
				)
				.append(
					"<span><i class='material-icons deleteicon'>indeterminate_check_box</i></span>"
				)
				.click(function() {
					count -= 1;
					console.log(count);
...
$(function() {
	$("#selectallbtn").click(function() {
		...
		$("#mainItem a").each(function() {
			...
			clone
				.prepend(
					"<i class='material-icons checkedboxicon'>check_box</i>"
				)
				.append(
					"<span><i class='material-icons deleteicon'>indeterminate_check_box</i></span>"
				)
...
// query variables part ends

// Here is we need another one to forcus on the trigger events part operation
var trigger = [];
...
$(function() {
	...
	$("#mainTrigger")
		...
		.click(function() {
			...
			// clone.append("<span style='color:red; font-weight:bold; cursor:pointer'>X</span>")
			clone.children("i").remove();
			var trigger_text = clone.text();
			console.log(trigger_text);
			// variable[variable_text]="in"; //add a to the varaible list dictionary
			trigger.push(trigger_text);
			document.getElementById("selected_trigger_event").value = trigger;
			console.log(trigger);

			$(".conditional_statement_varaible_select").empty();
			$.each(trigger, function(i, p) {
				console.log(p);
				// $('#conditional_statement_varaible_select').append($('<a class="dropdown-item"></a>').val(p).html(p));
				// 这里去掉了.val(p)的部分
				$(".conditional_statement_varaible_select").append(
			...
			clone
				...
				.click(function() {
					...
					// console.log(mainclick_text);
					// delete variable[variable_text];  //delete the element from the variable list dictionary
					trigger.splice(trigger.indexOf(trigger_text), 1);
					document.getElementById(
						"selected_trigger_event"
					).value = trigger;
					console.log(trigger);
					$(this).remove("a");
					...
					for (var i = 0; i < trigger.length; i++) {
						json_trans["position" + (i + 1)] = trigger[i];
					}
					console.log(json_trans);
					var x = JSON.stringify(json_trans);
					console.log(x);
...
$(function() {
	$("#triggerallbtn").click(function() {
		$("#whole-conditional-statements-box").show();
		$("#conditionalinstruction").hide();
		$("#mainTrigger a").each(function() {
			var clone = $(this).clone();
			var mainclick = $(this);
			// variables.push($(this).val());
			clone.children("i").remove();
			var trigger_text = clone.text(); //new
			trigger.push(trigger_text); //new
			document.getElementById("selected_trigger_event").value = trigger;
			console.log(trigger);
			clone
				.prepend(
...
$(function() {
	$("#cleartriggerbtn").click(function() {
		$("#whole-conditional-statements-box").hide();
		$("#conditionalinstruction").show();
		$("#selectedTrigger a").each(function() {
			$(this).remove();
		});
		$("#mainTrigger a").show();
		trigger = [];
			...
			if (selections.items.length) {
				...
				for (var len = items.length, i = 0; i < len; i++) {
					if (!items[i].getAttribute("aria-grabbed")) {
						...
						items[i].getAttribute("aria-grabbed") == "true"
					) {
						items[i].setAttribute("tabindex", "0");
					}
				}
				...
				if (e.target.getAttribute("aria-grabbed")) {
					...
					if (e.keyCode == 77 && hasModifier(e)) {
						...
						if (selections.items.length) {
							...
							if (
								selections.owner == targets[targets.length - 1]
							) {
								targets[0].focus();
							}
		...
		document.addEventListener(
			...
			function(e) {
				...
				if (selections.droptarget) {
					...
					for (
						var len = selections.items.length, i = 0;
						i < len;
						i++
					) {
...
$(function() {
	$(".add-one-group-btn").click(function() {
		$(this)
			...
			.siblings(".statement-part")
			.append(
				'<div class="conditionalstatements container col-md-12" id="conditionalstatements">\
